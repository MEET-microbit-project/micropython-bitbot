...
STATIC mp_obj_t bitbot_set_speed(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args)
{
  ...
  if (speed < 0) {
    speed_exact = 1023 * (1 + speed / 100.0);
    // microbit_pin_write_digital(PIN_MOTOR_LEFT_DIR, mp_const_int_one);
  } else {
    speed_exact = 1023 * (speed / 100.0);
    // microbit_pin_write_digital((microbit_pin_obj_t*)&microbit_p8_obj, mp_const_int_zero);
    microbit_pin_write_digital(PIN_BUZZER, mp_const_int_one);
    microbit_sleep(mp_obj_new_int(100));
    microbit_pin_write_digital(PIN_BUZZER, mp_const_int_zero);
  }
  // microbit_pin_write_analog(PIN_MOTOR_LEFT_SPEED, mp_obj_new_int(speed_exact));
  return mp_const_none;
